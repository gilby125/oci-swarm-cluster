...
init_swarm_manager() {
    ...
    retry_command "docker swarm init --advertise-addr $private_ip" "Swarm initialization"
...
create_overlay_networks() {
    ...
    if ! docker network ls | grep -q "agent_network"; then
        ...
        retry_command "docker network create --driver=overlay --attachable agent_network" "Create agent_network"
        if [ $? -ne 0 ]; then
    ...
    if ! docker network ls | grep -q "traefik-public"; then
        ...
        retry_command "docker network create --driver=overlay --attachable traefik-public" "Create traefik-public network"
        if [ $? -ne 0 ]; then
            log_message "WARN" "Failed to create traefik-public network. Trying with a different name..."
            # Try with swarm_ prefix
            retry_command "docker network create --driver=overlay --attachable swarm_traefik-public" "Create swarm_traefik-public network"
            if [ $? -ne 0 ]; then
...
# Always run the main function when the script is executed
init_docker_swarm "$@"
